...
if(!([_unit,primaryWeapon _unit] call dcd_suck_fnc_isSupported)) exitWith{0};

if(count _oldUnitLoadout == 0) then
{
	["new loadout vaildating ...","onLoadoutChanged"] call dcd_suck_fnc_debugOut;
...
else
{
    ...
    [("cahched current Weapon: " + str _currentWeapon),"onLoadoutChanged"] call dcd_suck_fnc_debugOut;
